...
import { fetchEstoqueCriticoApi } from '@/services/businessLogicApi';
...
export default function WidgetEstoqueCritico() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await fetchEstoqueCriticoApi();
        if (!m) return;
        setCritico(rows || []);
      } catch {
        if (!m) return;
      ...
      <div className="flex-1 overflow-auto">
        {critico.length === 0 ? (
          ...
          <div className="divide-y divide-border">
            {critico.slice(0, 6).map((p, idx) => {
              const pct =
                p.estoque_minimo > 0
                  ? Math.min(100, (p.estoque_atual / p.estoque_minimo) * 100)
                  : 0;
              return (
                <div key={p.codigo || p.id || idx} className="px-3 py-2">
