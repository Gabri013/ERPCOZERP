...
const EMPTY = {
  codigo:'', razao_social:'', nome_fantasia:'', tipo:'PJ', cnpj_cpf:'',
  email:'', telefone:'', celular:'', cep:'', endereco:'', numero:'',
  bairro:'', cidade:'', estado:'SP', contato:'', limite_credito:'', status:'Ativo', observacoes:''
};
...
export default function ModalCliente({ cliente, onClose, onSave }) {
  ...
  return (
    <FormModal title={cliente ? `Editar Cliente — ${cliente.razao_social}` : 'Novo Cliente'}
      onClose={onClose} onSave={handleSave} saving={saving} size="lg">
      <div className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="col-span-2">
            <label className={lbl}>Razão Social {req}</label>
            <input className={inp} value={form.razao_social} onChange={e=>upd('razao_social',e.target.value)}/>
          </div>
          <div>
            <label className={lbl}>Nome Fantasia</label>
            <input className={inp} value={form.nome_fantasia} onChange={e=>upd('nome_fantasia',e.target.value)}/>
          </div>
          <div>
            <label className={lbl}>{form.tipo==='PJ'?'CNPJ':'CPF'}</label>
            <input className={inp} value={form.cnpj_cpf} onChange={e=>upd('cnpj_cpf',e.target.value)} placeholder={form.tipo==='PJ'?'00.000.000/0001-00':'000.000.000-00'}/>
          </div>
          <div>
        ...
        <div className="border-t border-border pt-3">
          <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-2">Contato</div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div><label className={lbl}>Celular</label><input className={inp} value={form.celular} onChange={e=>upd('celular',e.target.value)}/></div>
            <div><label className={lbl}>Contato</label><input className={inp} value={form.contato} onChange={e=>upd('contato',e.target.value)} placeholder="Nome do contato"/></div>
            <div><label className={lbl}>Limite de Crédito (R$)</label><input type="number" className={inp} value={form.limite_credito} onChange={e=>upd('limite_credito',e.target.value)}/></div>
          </div>
        </div>
        <div className="border-t border-border pt-3">
          <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-2">Endereço</div>
          <div className="grid grid-cols-4 gap-3">
            ...
            <div><label className={lbl}>Número</label><input className={inp} value={form.numero} onChange={e=>upd('numero',e.target.value)}/></div>
