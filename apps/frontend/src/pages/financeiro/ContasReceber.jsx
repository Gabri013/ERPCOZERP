...
export default function ContasReceber() {
  ...
  const handleSave = async (form) => {
    if (editando?.id) {
      await contasReceberService.update(editando.id, { ...editando, ...form });
    } else {
  ...
  const marcarPago = async (id) => {
    ...
    if (!target) return;
    await contasReceberService.update(id, { ...target, status: 'Pago' });
    await reload();
    setDetalhe(null);
  };
          ...
          <div className="mt-3 flex flex-col sm:flex-row justify-end gap-2">
            ...
            <button
              onClick={async () => {
                ...
                await contasReceberService.delete(detalhe.id);
                setDetalhe(null);
                await reload();
              }}
