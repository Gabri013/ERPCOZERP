...
export default function Produtos() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/estoque?${params.toString()}`);
      setData(res?.data || []);
    } catch (err) {
  ...
  const handleSave = async (form) => {
    if (editando?.id) {
      const res = await api.put(`/api/estoque/${editando.id}`, form);
      const updated = res?.data;
      setData((prev) => prev.map((p) => (p.id === updated.id ? updated : p)));
    } else {
      const res = await api.post('/api/estoque', form);
  ...
  const handleDelete = async (produto) => {
    ...
    if (!confirm(`Excluir ${produto.codigo || ''} — ${produto.descricao || ''}?`)) return;
    await api.delete(`/api/estoque/${produto.id}`);
    setDetalhe(null);
    setData((prev) => prev.filter((p) => p.id !== produto.id));
  };

  const filtered = useMemo(() => {
    return data.filter((p) => (!filters.status || p.status === filters.status));
  }, [data, filters.status]);
